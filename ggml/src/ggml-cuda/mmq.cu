...
void ggml_cuda_mul_mat_q(
...

        {
            ...
            if (use_native_fp4) {
                if (src0->type == GGML_TYPE_MXFP4) {
                    ...
                    static_assert(sizeof(block_fp4_nvfp4_mmq) == 4 * sizeof(block_q8_1));
...

    {
        ...
        if (use_native_fp4) {
            if (src0->type == GGML_TYPE_MXFP4) {
                ...
                quantize_mmq_nvfp4_cuda(src1_d, ids_src1.get(), src1_q8_1.get(), src0->type, ne10, s11, s12, s13,
                                        ne10_padded, ne11_flat, ne12_flat, ne13_flat, stream);
            }
        } else {
    ...
    const int64_t s12 = use_native_fp4 ? ne11 * ne10_padded * sizeof(block_q8_1) / (QK8_1 * sizeof(int)) :
                                           ne11 * ne10_padded * sizeof(block_q8_1) / (QK8_1 * sizeof(int));
    const int64_t s13 = ne12*s12;
